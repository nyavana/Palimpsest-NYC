...
The current project version extends the base minimum viable product in three important ways. First, the map is no longer a passive visualization; citations, route stops, and markers now highlight one another bidirectionally. Second, the interface supports true multi-turn sessions, so follow-up instructions such as ``make it shorter'' or ``focus on churches'' operate on preserved history rather than starting a new request from scratch. Third, the system supports a structured food discovery flow, allowing a user to ask for coffee, lunch, or dessert nearby, inspect candidate places on the map, and continue the walk to a chosen stop. Together, these extensions move the project closer to a practical city companion while preserving the original grounding guarantees.

\section{Related Work and Background}
Palimpsest NYC sits at the intersection of retrieval-augmented generation, interactive map systems, and route planning. Retrieval-augmented generation (RAG) systems commonly improve factuality by providing documents to an LLM at answer time, but many still leave the final response weakly grounded: the model can mention items that were never actually retrieved, or it can cite loosely. Our design therefore emphasizes a \emph{verified citation contract}: every output citation must match a document that appeared in the active conversation ledger.

On the spatial side, modern map applications often rely on a dedicated routing backend instead of asking the LLM to improvise paths. We follow that design through OSRM, which computes street-following walking routes and returns structured geometry and turn-by-turn steps. This separation of concerns is important: the LLM decides when a route is needed and which place IDs matter, while the graph-routing backend handles route optimization and path generation deterministically.

The project also draws on public geographic and encyclopedic data. OpenStreetMap provides named places, amenities, and coordinates; Wikipedia and Wikidata provide richer historical context and source provenance. A bounded geography keeps the retrieval problem tractable and aligned with the project goal. Instead of solving open-world search, the system solves a much narrower question well: how to build a reliable, navigable narrative over a small but information-dense urban region.
...
\noindent\textbf{GitHub repository:} \url{https://github.com/DANNNNNJUAN/Palimpsest-NYC_PRO}

\noindent\textbf{Demo video:} \url{https://INSERT-DEMO-VIDEO-LINK-HERE}
