...
A \texttt{RetrievalMode} factory reads the \texttt{RETRIEVAL\_MODE} env flag at boot and selects one of three implementations. The \emph{dense} mode runs a pgvector cosine search over 384-dim \texttt{BAAI/bge-small-en-v1.5} embeddings indexed with ivfflat (\texttt{lists=100})~\cite{bge}. The \emph{hybrid} mode adds a sparse branch over \texttt{pg\_trgm} name similarity~\cite{pgtrgm} and fuses the two rankings with Reciprocal Rank Fusion at \(k=60\)~\cite{rrf}. The \emph{hybrid-reranked} mode keeps that fusion step and then re-scores the top-\(k\) union with a BGE-reranker-base cross-encoder~\cite{bgereranker}. All three modes return the same \texttt{SearchPlaceHit} tool-result shape, so the agent loop never sees the mode: swapping it changes ranking quality, not the tool contract or the verifier path.
...
The corpus has been widened from the original Morningside Heights / Upper West Side slice to all of Manhattan. Ingestion runs against the bounding box \((-74.02, 40.70, -73.91, 40.88)\) under \texttt{SCOPE\_VERSION = v2-manhattan}, and after de-duplication yields \textbf{12{,}858} OSM places, \textbf{492} Wikipedia places, and \textbf{456} Wikipedia documents. On the OSM side that is a \(\mathbf{9.4\times}\) expansion over the V1 MH/UWS slice (1{,}363 places). The Wikipedia side is flat at 492 because the Wikipedia ingest is API-capped at 500 items per batch; the cap binds before the geographic filter, so the 492 final count matching the V1 figure is a cap artifact rather than a coincidence of geography. Lifting the cap is a v3 follow-up. Table~\ref{tab:corpus} reports the counts.
...
The LLM call path has two router tiers, \texttt{local} and \texttt{openrouter}, each behind its own circuit breaker configured for \textbf{3 failures} inside a \textbf{60-second window} with a \textbf{30-second cooldown}. Bring-your-own-key (BYOK) requests carry credentials in an \texttt{X-LLM-Credentials} header; the backend reads that header per request and constructs a \emph{per-session} router with fresh breaker state and an isolated cache namespace, so a user-supplied bad key cannot trip the shared breaker or poison the shared LLM cache for other sessions. The agent loop also gives \texttt{plan\_walk} an along-route discovery step: alongside the OSRM walking geometry between cited places, it surfaces nearby POIs as candidate stops on that route. This folds route-aware discovery into the existing two-tool surface rather than adding a third tool, and the discovered stops flow through the same citation ledger as any other retrieved place.
...
The headline finding from the ablation is that turning on the cross-encoder reranker on top of hybrid retrieval moves CCR by only +0.4\,pp (within CI) while regressing HR by +17.4\,pp and dropping out-of-scope refusal (GRR) from 0.500 to 0.200. CCR stays roughly flat because the reranker reshuffles top-k order without changing which documents are available. HR rises because the agent reads the reshuffled ordering as license to make more, longer factual claims that it then cannot ground in the OSM corpus. The takeaway is that reranker tuning in an agent-grounded RAG pipeline is an \emph{agent-prompt} concern, not just a retrieval-quality concern: the same retrieval set can produce very different grounding behavior depending on what the agent is encouraged to do with it. Plain hybrid remains the strongest Palimpsest configuration for citation grounding in this run.

Four evaluation caveats matter. The judge is \texttt{gpt-5.4-mini}, the same model that drives the systems under test, which is a known self-grading risk we accepted to keep the judge bill near \$1.60 across 1550 calls; inter-rater $\kappa$ against hand grades is deferred (the \texttt{kappa} cell is \texttt{null} in the aggregate table, recoverable by re-running \texttt{aggregate.py --hand-grades}). The Redis-backed LLMCache contaminates wall-clock latency and cost across re-runs of the same question but does not affect the correctness metrics (CCR, HR, FA, NQ, GRR), which evaluate the same answer either way. FA is CI-noisy at $n=95$, so the reranker's FA = 1.18 against hybrid's FA = 1.42 should be read as suggestive rather than decisive.\footnote{See \texttt{docs/eval/v1-eval-report.md} for the V1 Kimi K2.6 vs.\ GPT-OSS-120B router-bench (cost/latency, different question set; not reported here).}
...
\newblock \url{https://arxiv.org/abs/2309.07597}

\bibitem{rrf}
Gordon V. Cormack, Charles L. A. Clarke, and Stefan B\"{u}ttcher.
\newblock Reciprocal Rank Fusion outperforms Condorcet and individual rank learning methods.
\newblock In \emph{Proceedings of the 32nd International ACM SIGIR Conference on Research and Development in Information Retrieval (SIGIR '09)}, pages 758--759, 2009.
...
\bibitem{pgtrgm}
The PostgreSQL Global Development Group.
\newblock pg\_trgm --- support for similarity of text using trigram matching.
\newblock PostgreSQL documentation.
\newblock \url{https://www.postgresql.org/docs/current/pgtrgm.html}

\bibitem{bgereranker}
Shitao Xiao, Zheng Liu, Peitian Zhang, and Niklas Muennighoff.
\newblock BGE-reranker-base: a cross-encoder reranker from the FlagEmbedding project.
\newblock BAAI / Hugging Face model card.
\newblock \url{https://huggingface.co/BAAI/bge-reranker-base}

\bibitem{overpass}
OpenStreetMap Foundation.
\newblock Overpass API.
\newblock \url{https://overpass-api.de/}
