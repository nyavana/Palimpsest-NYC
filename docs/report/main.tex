...
\title{Palimpsest NYC: A Citation-Grounded Agentic Walking-Tour System for Manhattan}
...
The ablation compares five systems: \texttt{vanilla} (LLM with no retrieval), \texttt{naive\_rag} (top-$k$ retrieval concatenated into the prompt), \texttt{palimpsest-dense}, \texttt{palimpsest-hybrid}, and \texttt{palimpsest-hybrid-\allowbreak reranked}. All five are served by \texttt{openai/\allowbreak gpt-5.4-mini} through OpenRouter, so the model is held constant; the three Palimpsest variants differ only by the \texttt{RETRIEVAL\_MODE} boot flag and the reranker-enabled toggle.
...
\begin{table*}[t]
  ...
  \begin{tabular}{l r l l l l}
    ...
    \texttt{palimpsest-hybrid-\allowbreak reranked} & 95 & 0.755 [0.678, 0.818] & 0.794 [0.722, 0.860] & 3.52 [3.39, 3.64] & 0.200 [0.000, 0.500] \\
    \bottomrule
  \end{tabular}
\end{table*}
...
\begin{itemize}[leftmargin=*,itemsep=2pt,topsep=2pt]
  ...
  \item \textbf{Doc-ID harvesting.} We reconstruct \texttt{retrieved\_docs} from the terminal \texttt{citations} frame and from \texttt{walk.\allowbreak discovered\_stops[]}, not from a tool-result ledger: the V1 SSE schema only exposes \texttt{\{name, n\_hits\}} per tool call. This undercounts the HR denominator, because any retrieved-but-uncited document is invisible to the eval.
  ...
  \item \textbf{Inter-rater $\kappa$ deferred.} The \texttt{kappa} cell in the ablation table is \texttt{null}. Hand-grading the calibration worksheet at \texttt{docs/\allowbreak eval/\allowbreak grades/\allowbreak calibration.csv} and re-running \texttt{aggregate.py --hand-grades} recovers the number; we defer that step in this revision.
\end{itemize}
...
\noindent\textbf{Demo video:} \url{https://drive.google.com/drive/folders/1mjrAE5U0Nncnt1NoWQfJhvfDCS31U4H8}
