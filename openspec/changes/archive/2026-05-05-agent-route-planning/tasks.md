...
- [x] 10.7 Frontend dev server (`npm run dev`) renders the new walk view correctly for a recorded tour fixture; confirm map path follows streets, timeline shows totals, per-stop disclosure expands turn-by-turn. *Verified manually post-rebase: stack rebuilt with rebased code, browser at `:5173` rendered street-following path, totals footer, and expandable turn-by-turn for a 3-stop Morningside Heights tour. Marker popups + 2D/3D toggle from #5 confirmed working alongside.*
...
- [x] 12.1 Add a section to root `README.md` describing the new `osrm` and `osrm-prepare` services and the one-time bbox extract step. *Added "Routing graph (OSRM)" subsection under "Build from source" + a haversine-fallback callout under "What's in the published images". Detailed extract refresh procedure links out to `infra/osrm/README.md` to avoid duplication.*
...
- [x] 12.3 Add a short dated note `docs/route-planning-2026-05-04.md` describing the OSRM choice (with `/route` vs `/trip` branching), the GeoJSON-not-polyline rationale, the walk-intent soft-hint pattern, the OSM extract refresh procedure, and the `RoutingBackend` swap-in pattern. *Added; mirrors the format of `docs/swap-llm-tiers-2026-04-28.md` (architecture decisions / tests / what unblocks next / out-of-scope sections). Includes the `radiuses=...` removal lesson learned during implementation.*
